                ...
                <form id="formUpload" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mt-3">
                            ...
                            <input type="hidden" name="id_visi" id="id_visi" class="form-control" value="{{ $visi_misi->id }}">
                        </div>

                      
                        <div class="col-md-3 mt-3">
                            <label>Keterangan </label>
                            <textarea name="keterangan" id="keterangan" class="form-control" rows="5">{{ old('keterangan', $visi_misi->keterangan) }}</textarea>
                        ...
                        <div class="col-md-3 mt-3">
                            ...
                            <select name="status1" id="status1" class="form-control" required>
                                ...
                                <option value="1" {{ $visi_misi->status1 == 1 ? 'selected' : '' }}>Aktif</option>
                                <option value="2" {{ $visi_misi->status1 == 2 ? 'selected' : '' }}>Nonaktif</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-12 mt-5">                                                  
                            <label>Visi </label>
                            <textarea id="visi_editor" name="visi">{{ old('visi_editor', $visi_misi->visi) }}</textarea>
                           
                        </div>

                        <div class="col-md-12 mt-3">                                                  
                            <label>Misi </label>
                            <textarea id="misi_editor" name="misi" placeholder="Masukkan Misi">{{ old('misi_editor', $visi_misi->misi) }}</textarea>
...
<script>
    ...
    const id = $('#id_visi').val();

    formData.append('_token', '{{ csrf_token() }}');
    formData.append('_method', 'PUT'); // spoofing method here
    ...
    $.ajax({
        ...
        success: (response) => {
            if (response.code === 200) {
                Swal.fire('Success', 'Visi berhasil disimpan!', 'success').then(() => {
                    window.location = '{{ url("/desa_visi") }}';
                });
            } else {
                Swal.fire({
                    icon: 'error',
