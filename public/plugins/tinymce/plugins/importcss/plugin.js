(function () {
    var importcss = (function () {
        ...
        var isExclusiveMode = function (editor, group) {
            return (
                group === null ||
                $_33utw3fcjkmcdvda.shouldImportExclusive(editor) !== false
            );
        };
        ...
        var convertSelectorToFormat = function (
            editor,
            plugin,
            selector,
            group,
            ...
            if (group && group.selector_converter) {
                ...
                selectorConverter =
                    $_33utw3fcjkmcdvda.getSelectorConverter(editor);
            } else {
        ...
        var setup = function (editor) {
            editor.on("renderFormatsMenu", function (e) {
                ...
                var selectorFilter = compileFilter(
                        $_33utw3fcjkmcdvda.getSelectorFilter(editor),
                    ),
                    ctrl = e.control;
                var groups = compileUserDefinedGroups(
                    $_33utw3fcjkmcdvda.getCssGroups(editor),
                );
                var processSelector = function (selector, group) {
                    if (
                        ...
                        if (format) {
                            var formatName =
                                format.name || global$1.DOM.uniqueId();
                ...
                global$4.each(
                    getSelectors(
                        editor,
                        e.doc || editor.getDoc(),
                        compileFilter($_33utw3fcjkmcdvda.getFileFilter(editor)),
                    ),
                    function (selector) {
                        if (selector.indexOf(".mce-") === -1) {
                            if (!selectorFilter || selectorFilter(selector)) {
                                var selectorGroups = getGroupsBySelector(
                                    groups,
                                    selector,
                                );
        ...
        var get = function (editor) {
            var convertSelectorToFormat = function (selectorText) {
                return $_1uu6rcf7jkmcdvd3.defaultConvertSelectorToFormat(
                    editor,
                    selectorText,
                );
            };
