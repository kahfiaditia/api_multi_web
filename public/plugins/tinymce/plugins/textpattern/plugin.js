(function () {
    var textpattern = (function () {
        ...
        var hasContent = function (offset, delta, pattern) {
            return (
                offset - delta - pattern.end.length - pattern.start.length > 0
            );
        };
        ...
        var patternFromRng = function (patterns, rng, space) {
            ...
            var endPattern = $_9lo27ss1jkmcdy09.findEndPattern(
                patterns,
                text,
                rng.startOffset,
                delta,
            ...
            var endOffset = text.lastIndexOf(
                endPattern.end,
                rng.startOffset - delta,
            );
            var startOffset = text.lastIndexOf(
                endPattern.start,
                endOffset - endPattern.end.length,
            );
            endOffset = text.indexOf(
                endPattern.end,
                startOffset + endPattern.start.length,
            );
            if (startOffset === -1) {
                return;
            }
            ...
            var startPattern = $_9lo27ss1jkmcdy09.findPattern(
                patterns,
                patternRng.toString(),
            );
            if (
                ...
                container.data.length <=
                    endPattern.start.length + endPattern.end.length
            ) {
                return;
            }
            return {
        ...
        var splitAndApply = function (editor, container, found, space) {
            var formatArray = global$4.isArray(found.pattern.format)
                ? found.pattern.format
                : [found.pattern.format];
            var validFormats = global$4.grep(
                formatArray,
                function (formatName) {
        ...
        var applyBlockFormat = function (editor, patterns) {
            ...
            if (textBlockElm) {
                ...
                if (firstTextNode) {
                    pattern = $_9lo27ss1jkmcdy09.findPattern(
                        patterns,
                        firstTextNode.data,
                    );
                    if (!pattern) {
                    ...
                    if (
                        global$4.trim(firstTextNode.data).length ===
        ...
        function handleEnter(editor, patterns) {
            ...
            wrappedTextNode = $_5h80lyryjkmcdy04.applyInlineFormatEnter(
                editor,
                patterns,
            );
            if (wrappedTextNode) {
        ...
        function handleInlineKey(editor, patterns) {
            ...
            wrappedTextNode = $_5h80lyryjkmcdy04.applyInlineFormatSpace(
                editor,
                patterns,
            );
            if (wrappedTextNode) {
                ...
                if (/[\u00a0 ]/.test(lastChar)) {
                    wrappedTextNode.deleteData(
                        wrappedTextNode.data.length - 1,
                        1,
                    );
        ...
        var checkKeyCode = function (codes, event) {
            return checkKeyEvent(codes, event, function (code, event) {
                return (
                    code === event.keyCode &&
                    global$2.modifierPressed(event) === false
                );
            });
        };
        ...
        var setup = function (editor, patternsState) {
            ...
            editor.on("keyup", function (e) {
                if ($_2jhe9qrxjkmcdy01.checkKeyCode(keyCodes, e)) {
                    $_2jhe9qrxjkmcdy01.handleInlineKey(
                        editor,
                        patternsState.get(),
                    );
                }
        ...
        global.add("textpattern", function (editor) {
            var patternsState = Cell(
                $_4khy2ortjkmcdxzy.getPatterns(editor.settings),
            );
